...
let handler = async (m, {
	...
	if (isOwner) {
		...
		await delay(1500)
		conn.reply(res, `*${conn.user.name}* adalah bot whatsapp yang di bangun menggunakan Nodejs, diundang oleh Владелец kami *@${m.sender.split`@`[0]}* \n\nUntuk Melihat List *Menu* bot ketik *#menu*\n`.trim(), null, {
			mentions: [m.sender]
		})
